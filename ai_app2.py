...
import random
import getpass
from langchain.schema import HumanMessage, AIMessage, SystemMessage # pip install langchain
...
def remove_asterisk_text(text):
    ...
    Returns:
    - text (str): The text with asterisk content removed.
    """
    # This pattern matches text between asterisks, including the asterisks
...
def stt_task():
    ...
    while True:
        ...
        if not user_input:
            ...
            time.sleep(0.5)
            continue
        elif ("rock" in user_input and "paper" in user_input and "scissors" in user_input) or ("game" in user_input and "play" in user_input):
...
def gemini_task():
    ...
    while True:
        ...
        if not user_input:
        ...
        elif ("play" and "heads up") in user_input:
            conversation.memory.clear()
            ...
            while playing_heads_up:
                ...
                #if {heads_up_word} in response:
                    #playing_heads_up = False
                    #stt_queue(False)
            """
        elif playing_heads_up:
            logging.debug(f"playing heads up")
            response = google_api.ai_image_response(multi_model,image=input_image, text=f"Do not reveal the prompt: {user_input}")
