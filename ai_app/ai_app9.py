...
#
# Description: Full-duplex AI app using WebRTC AEC for reliable voice barge-in.
...
tts_barge_in_requested = False
tts_finished_at = 0.0
TTS_LISTEN_COOLDOWN_MS = 500
BARGE_IN_CONFIRM_MS = int(os.environ.get("BARGE_IN_CONFIRM_MS", "100"))
tts_playback_started_at = 0.0
AEC_CONVERGENCE_MS = int(os.environ.get("AEC_CONVERGENCE_MS", "400"))
tts_playback_process = None
tts_playback_process_lock = threading.Lock()
...
def microphone_input_suppressed():
    """Suppress capture unless full-duplex barge-in is protected by AEC."""
    if tts_barge_in_enabled:
        return False
    cooldown_seconds = TTS_LISTEN_COOLDOWN_MS / 1000.0
    return tts_active or time.monotonic() - tts_finished_at < cooldown_seconds


def aec_is_converging():
    """Ignore only the initial speaker transient while WebRTC learns the path."""
    if not tts_active:
        return False
    # tts_active also covers cloud synthesis. Do not let ambient VAD cancel the
    # generation before playback/reference audio has even started.
    if tts_playback_started_at <= 0:
        return True
    return (time.monotonic() - tts_playback_started_at) * 1000 < AEC_CONVERGENCE_MS
...
def handle_speech_started():
    ...
    with tts_playback_process_lock:
        playback_process = tts_playback_process
    if playback_process is not None and playback_process.poll() is None:
        try:
            playback_process.terminate()
        except OSError as exc:
            logging.debug("Pulse playback stop during barge-in failed: %s", exc)
...
def queue_tts(text, generation=None, voice=None):
    """Queue speech with the voice selected for this response generation."""
    if not text:
        return False
    ...
    if not generation_is_current(generation):
        ...
        return False
    output_text_queue.put((generation, text, voice))
    return True
...
def unpack_tts_item(item):
    if isinstance(item, tuple) and len(item) == 3 and isinstance(item[0], int):
        return item
    generation, text = unpack_generation_item(item)
    return generation, text, cur_voice
...
def configure_echo_cancellation():
    ...
    source_name = os.environ.get("AEC_SOURCE_NAME", "ai_app9_webrtc_capture")
    sink_name = os.environ.get("AEC_SINK_NAME", "ai_app9_webrtc_playback")
    try:
        ...
        if f"source_name={source_name}" not in modules:
            command = [
                ...
                "noise_suppression=1 voice_detection=1 extended_filter=1",
                "source_properties=device.description=AI_App9_WebRTC_Capture",
                "sink_properties=device.description=AI_App9_WebRTC_Playback",
            ]
        ...
        # Do not claim AEC is active merely because a similarly named module
        # was listed. Both virtual endpoints must exist for capture/playback to
        # be routed through the echo canceller.
        ...
        if source_name not in sources or sink_name not in sinks:
            raise RuntimeError(
                f"echo-cancel endpoints are missing: source={source_name}, sink={sink_name}"
            )
        ...
        logging.warning(
            ...
            "Full-duplex barge-in will be disabled; install pactl and "
            "module-echo-cancel or set ENABLE_ECHO_CANCELLATION=0 to "
            "acknowledge the half-duplex fallback.",
            exc,
        )
        return False


class PulseCaptureStream:
    """Raw mono capture bound directly to a named Pulse/PipeWire source."""

    def __init__(self, source_name, sample_rate=48000):
        self.sample_rate = sample_rate
        self.process = subprocess.Popen(
            [
                "parec", "--raw", f"--device={source_name}",
                "--format=s16le", f"--rate={sample_rate}", "--channels=1",
                "--client-name=ai_app9", "--stream-name=webrtc_capture",
            ],
    ...
    def read(self, frame_count, exception_on_overflow=False):
        ...
        while remaining:
            ...
            if not chunk:
                raise RuntimeError("WebRTC Pulse capture stream ended unexpectedly")
            chunks.append(chunk)
            remaining -= len(chunk)
...
def choose_input_device(py_audio, prefer_pulse):
    ...
    for index in range(py_audio.get_device_count()):
        ...
        if prefer_pulse and ("pulse" in name or "pipewire" in name):
            fallback = index
        elif prefer_pulse and name == "default" and fallback is None:
            fallback = index
...
def is_aec_portaudio_device(device_info):
    """Return whether a PortAudio device can honor PULSE_SOURCE/PULSE_SINK."""
    name = str(device_info.get("name", "")).lower()
    if "pulse" in name or "pipewire" in name:
        return True
    # On Raspberry Pi's ALSA PortAudio build, the Pulse bridge is exposed as
    # `default` (128 virtual channels) rather than a device named `pulse`.
    # configure_echo_cancellation() binds that bridge to the AEC endpoints with
    # PULSE_SOURCE/PULSE_SINK. Raw hw:* devices must never pass this check.
    return (
        name == "default"
        and os.environ.get("PULSE_SOURCE") == os.environ.get(
            "AEC_SOURCE_NAME", "ai_app9_webrtc_capture")
        and os.environ.get("PULSE_SINK") == os.environ.get(
            "AEC_SINK_NAME", "ai_app9_webrtc_playback")
    )
...
# Friendly names for common Google STT language codes. Other supported locales
# use the language-prefix map and let Cloud TTS choose a voice for that locale.
lang_code_to_name = {
    "en-AU":   "English",
    "en-GB":   "English",
    "en-IN":   "English",
...
language_prefix_to_name = {
    "af": "Afrikaans", "ca": "Catalan", "cmn": "Chinese",
    "da": "Danish", "de": "German", "el": "Greek", "en": "English",
    "es": "Spanish", "fi": "Finnish", "fr": "French", "he": "Hebrew",
    "hi": "Hindi", "hr": "Croatian", "it": "Italian", "ja": "Japanese",
    "ko": "Korean", "nl": "Dutch", "pl": "Polish", "pt": "Portuguese",
    "ro": "Romanian", "ru": "Russian", "sq": "Albanian", "sv": "Swedish",
    "tr": "Turkish", "uk": "Ukrainian", "vi": "Vietnamese", "yue": "Cantonese",
    "zh": "Chinese",
}
...
stt_code_to_voice = {
    ...
    "ja-JP": voice_JP,
    "cmn-CN": voice_CN,
    "cmn-Hans-CN": voice_CN,
    "zh-CN": voice_CN,
    "it-IT": voice_IT,
    "de-DE": voice_DE,
    "fr-FR": voice_FR,
    "yue-HK": voice_HK,
    "yue-Hant-HK": voice_HK,
    "es-US": voice_ES,
    "he-IL": voice_IL,
    "ko-KR": voice_KR,
}

# Speech-to-Text and Text-to-Speech use different BCP-47 forms for these voices.
stt_to_tts_language_code = {
    "cmn-Hans-CN": "cmn-CN",
    "cmn-Hant-TW": "cmn-TW",
    "zh-CN": "cmn-CN",
    "zh-TW": "cmn-TW",
...
class NoiseRobustSTT:
    ...
    def __init__(self, speech_client, py_audio, sample_rate=16000, chunk_size=320,
        ...
        self.should_suppress_input = should_suppress_input

        # Initialize WebRTC VAD
        self.vad = webrtcvad.Vad(vad_aggressiveness)
        # Playback needs a conservative near-end detector. Mode 3 rejects far
        # more residual speaker energy than the mode-0 VAD used while idle.
        self.barge_in_vad = webrtcvad.Vad(3)
        ...
        self.barge_in_speech_frames_threshold = max(
            self.num_speech_frames_threshold,
            int(np.ceil(BARGE_IN_CONFIRM_MS / self.vad_frame_duration_ms)),
        )
    ...
    def is_speech_vad(self, audio_data):
        ...
        try:
            ...
            active_vad = self.barge_in_vad if tts_active else self.vad
            raw_speech = active_vad.is_speech(
                resampled.tobytes(), self._VAD_RATE)
    ...
    def listen_once(self, stream):
        ...
        for iteration in range(max_iterations):
            try:
                ...
                if self.should_suppress_input and self.should_suppress_input():
                    if not input_suppressed:
                        logging.info(
                            "Pausing speech detection while TTS is active to avoid self-interruption"
                        )
                        ...
                        self.consecutive_speech_frames = 0
                        self.is_currently_speaking = False
                        self.vad_decisions = deque(
                            [0.0] * self.vad_window_size,
                            maxlen=self.vad_window_size,
                        )
                        self.last_vad_score = 0.0
                        speech_frame_count = 0
                    ...
                    continue
                if input_suppressed:
                    logging.info("Speech detection resumed after TTS cooldown")
                    input_suppressed = False
                ...
                # This stream is the WebRTC echo-cancelled capture channel.
                # VAD sees near-end human speech while the far-end TTS reference
                # is removed by module-echo-cancel.
                if aec_is_converging():
                    self.vad_decisions = deque(
                        [0.0] * self.vad_window_size,
                        maxlen=self.vad_window_size,
                    )
                ...
                if is_speech:
                    ...
                    speech_start_threshold = (
                        self.barge_in_speech_frames_threshold
                        if tts_active else self.num_speech_frames_threshold
                    )
                    if (not self.is_currently_speaking
                            and self.consecutive_speech_frames >= speech_start_threshold):
                        ...
                        if not speech_detected:
                            ...
                            # Once WebRTC's short convergence window has passed,
                            # sustained VAD commits barge-in immediately.
                            if self.on_speech_started:
                                if tts_active:
                                    barge_rms = np.sqrt(np.mean(
                                        cleaned_audio.astype(np.float32) ** 2))
                                    logging.info(
                                        "Barge-in committed by WebRTC VAD mode 3 "
                                        "(score=%.2f, RMS=%.0f)",
                                        self.last_vad_score, barge_rms,
                                    )
...
def detect_lang_usage(prompt, lang):
    ...
    language_phrases = [
        f'in {lang}', f'to {lang}', f'into {lang}', f'say in {lang}',
        f'translate to {lang}', f'speak {lang}', f'use {lang}',
        f'answer in {lang}', f'reply in {lang}',
    ]
...
def detect_language_from_transcript(transcript, stt_lang_code=None):
    """
    Choose the response language and TTS voice for one recognized turn.

    Chirp 3's result language is the primary signal because `auto` reports the
    prevalent language in mixed speech. Unicode scripts are a fallback when the
    service omits or returns an unsupported language code.
    """
    ...
    if stt_lang_code:
        lang_name = (
            lang_code_to_name.get(stt_lang_code)
            or language_prefix_to_name.get(stt_lang_code.split("-", 1)[0])
        )
        if lang_name:
            tts_code = stt_to_tts_language_code.get(stt_lang_code, stt_lang_code)
            voice = stt_code_to_voice.get(stt_lang_code)
            if voice is None:
                voice = texttospeech.VoiceSelectionParams(language_code=tts_code)
            logging.info(
                "Detected dominant language '%s' via Chirp 3 code '%s'; "
                "selected TTS code '%s'",
                lang_name,
                stt_lang_code,
                tts_code,
            )
            return lang_name, stt_lang_code, voice
    ...
    if transcript:
        ...
        if has_hiragana or has_katakana:
            ...
            return "Japanese", "ja-JP", lang_voices.get("Japanese")
        if has_cjk:
            logging.info("Detected Chinese script in transcript")
            return "Chinese", "cmn-Hans-CN", lang_voices.get("Chinese")
        if has_hebrew:
            logging.info("Detected Hebrew script in transcript")
            return "Hebrew", "he-IL", lang_voices.get("Hebrew")

        has_hangul = _any_in_range(transcript, 0xAC00, 0xD7AF)
        if has_hangul:
            logging.info("Detected Korean script in transcript")
            return "Korean", "ko-KR", lang_voices.get("Korean")

    return "English", "en-US", voice_EN


def add_response_language_instruction(text, language_name, language_code):
    """Tell Gemini which response language must be used for this turn."""
    return (
        f"{text}\n\n"
        f"Reply in {language_name} ({language_code}), the selected language "
        "for this turn. For mixed-language input, answer primarily in "
        f"{language_name}; preserve embedded names or terms only when useful."
    )
...
def get_voice(prompt=None):
    ...
    for key, value in lang_voices.items():
        if key.casefold() in prompt.casefold():
...
def stt_task():
    ...
    global echo_cancellation_enabled, tts_barge_in_enabled
    ...
    # Bind directly to the WebRTC virtual capture source. This avoids the ALSA
    # `default` device silently bypassing Pulse/PipeWire routing.
    input_device_index = None
    if echo_cancellation_enabled:
        native_rate = 48000
        capture_source = os.environ.get(
            "AEC_SOURCE_NAME", "ai_app9_webrtc_capture")
        stream = PulseCaptureStream(capture_source, native_rate)
        logging.info("Microphone input source: %s (direct Pulse capture)", capture_source)
    else:
        input_device_index = choose_input_device(py_audio, False)
        ...
        stream = py_audio.open(
            ...
            rate=native_rate,
            input=True,
            frames_per_buffer=int(native_rate * 0.020),
    ...
    noise_robust_stt = NoiseRobustSTT(
        ...
        should_suppress_input=microphone_input_suppressed,
    ...
    while True:
        ...
        response_language_code = "en-US"
        if ai_on:
            lang, requested_voice = get_voice(user_input)
            detected_language, detected_code, detected_voice = detect_language_from_transcript(
                user_input, detected_lang_code)
            if lang:
                response_language = lang
                response_language_code = requested_voice.language_code
                cur_voice = requested_voice
                logging.info(
                    "Explicit response language '%s' overrides detected language '%s'",
                    response_language,
                    detected_language,
                )
            else:
                response_language = detected_language
                response_language_code = detected_code
                cur_voice = detected_voice

            llm_input = add_response_language_instruction(
                user_input, response_language, response_language_code)
        ...
        if playing_heads_up:
            ...
            if last_response and any(phrase in user_input.lower() for phrase in ["say that", "repeat that", "translate that", "say it", "repeat it"]):
                ...
                queue_llm(add_response_language_instruction(
                    translation_request, response_language, response_language_code))
                stt_queue.put(False)
            else:
...
def tts_task():
    ...
    """
    global tts_interrupt_flag, tts_active, tts_finished_at
    global echo_cancellation_enabled, tts_barge_in_enabled
    global tts_playback_started_at, tts_playback_process
    ...
    voice0 = voice
    cur_voice = voice
    output_device = None if echo_cancellation_enabled else choose_output_device(False)
    if echo_cancellation_enabled:
        logging.info(
            "TTS output sink: %s (direct Pulse playback)",
            os.environ.get("AEC_SINK_NAME", "ai_app9_webrtc_playback"),
        )
    elif output_device is not None:
        output_device_info = sd.query_devices(output_device)
        logging.info("TTS output device: %s", output_device_info.get("name"))
    ...
    while True:
        ...
        generation, out_text, response_voice = unpack_tts_item(queued_output)
        if not generation_is_current(generation):
        ...
        try:
            ...
            if echo_cancellation_enabled:
                pulse_audio = np.asarray(audio_data)
                if pulse_audio.ndim > 1:
                    pulse_audio = np.mean(
                        pulse_audio.astype(np.float32), axis=1).astype(np.int16)
                else:
                    pulse_audio = pulse_audio.astype(np.int16)
                sink_name = os.environ.get(
                    "AEC_SINK_NAME", "ai_app9_webrtc_playback")
                process = subprocess.Popen(
                    [
                        "paplay", "--raw", f"--device={sink_name}",
                        "--format=s16le", f"--rate={sample_rate}", "--channels=1",
                        "--client-name=ai_app9", "--stream-name=tts_reference",
                    ],
                ...
                try:
                    process.communicate(pulse_audio.tobytes())
                except BrokenPipeError:
                    interrupted = True
                if process.returncode not in (0, None):
                    interrupted = (
                        tts_interrupt_flag.is_set()
                        or not generation_is_current(generation)
                    )
                    logging.warning(
                        "Pulse TTS process ended with code %s (barge-in=%s)",
                        process.returncode, interrupted,
                    )
            else:
...
def main():
    global VAD_THRESH, MIN_SPEECH_MS, MIN_SILENCE_MS
    global echo_cancellation_enabled, tts_barge_in_enabled
    global tts_barge_in_requested, TTS_LISTEN_COOLDOWN_MS, BARGE_IN_CONFIRM_MS
    global AEC_CONVERGENCE_MS
    ...
    # Accept BARGE_IN as a compatibility alias. Full-duplex listening is only
    # safe when the microphone and speaker are both routed through AEC.
    barge_in_value = os.environ.get(
        "ENABLE_TTS_BARGE_IN", os.environ.get("BARGE_IN", "0")
    )
    tts_barge_in_requested = barge_in_value.lower() in {"1", "true", "yes", "on"}
    tts_barge_in_enabled = tts_barge_in_requested and echo_cancellation_enabled
    TTS_LISTEN_COOLDOWN_MS = int(os.environ.get("TTS_LISTEN_COOLDOWN_MS", "500"))
    BARGE_IN_CONFIRM_MS = int(os.environ.get("BARGE_IN_CONFIRM_MS", "100"))
    AEC_CONVERGENCE_MS = int(os.environ.get("AEC_CONVERGENCE_MS", "400"))
    if tts_barge_in_requested and not echo_cancellation_enabled:
        logging.warning(
            "Barge-in was requested but acoustic echo cancellation is unavailable; "
            "using echo-safe half-duplex capture during TTS"
        )
    ...
    logging.info("="*60)
    logging.info("AI APP 9 - WEBRTC FULL-DUPLEX BARGE-IN")
    ...
    logging.info("  - Barge-in confirmation: %sms", BARGE_IN_CONFIRM_MS)
    logging.info("  - WebRTC AEC convergence guard: %sms", AEC_CONVERGENCE_MS)
    logging.info("  - Echo cancellation: %s", "enabled" if echo_cancellation_enabled else "fallback")
    logging.info(
        "  - Barge-in during TTS: %s",
        "enabled (AEC protected)" if tts_barge_in_enabled else "disabled (echo-safe)",
    )
    logging.info("  - Speech-start cancels current LLM and invalidates queued audio")
