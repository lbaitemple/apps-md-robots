...
class NoiseRobustSTT:
    ...
    def __init__(self, speech_client, py_audio, sample_rate=16000, chunk_size=320,
        ...
        # Ambient noise floor, tracked continuously so a sudden noise (fan
        # kicking on, traffic, a door) raises the RMS gates used below rather
        # than being mistaken for speech until the static thresholds catch up.
        # Rising fast and falling slow means a burst of noise is absorbed in a
        # few frames, while the floor doesn't chase a real utterance back down.
        self.noise_floor = float(os.environ.get("NOISE_FLOOR_INIT_RMS", self.silence_threshold / 3))
        self.noise_floor_min = float(os.environ.get("NOISE_FLOOR_MIN_RMS", "50"))
        self.noise_floor_max = float(os.environ.get("NOISE_FLOOR_MAX_RMS", "4000"))
        self.noise_floor_rise_alpha = float(os.environ.get("NOISE_FLOOR_RISE_ALPHA", "0.3"))
        self.noise_floor_fall_alpha = float(os.environ.get("NOISE_FLOOR_FALL_ALPHA", "0.02"))
        self.barge_in_margin_rms = max(0, int(os.environ.get("BARGE_IN_MARGIN_RMS", "60")))
        # Baseline gate for normal listening (no TTS/echo residual to fight),
        # so it can sit well below the barge-in gate above.
        self.speech_min_rms = max(0, int(os.environ.get("SPEECH_MIN_RMS", "60")))
        self.speech_margin_rms = max(0, int(os.environ.get("SPEECH_NOISE_MARGIN_RMS", "80")))
        ...
        logging.info(
            ...
            "speech min RMS=%d, barge min RMS=%d "
            "(adaptive noise floor start=%.0f, range=%.0f-%.0f), "
            "end silence=%dms",
            ...
            self.speech_min_rms, self.barge_in_min_rms,
            self.noise_floor, self.noise_floor_min, self.noise_floor_max,
            self.num_silent_frames_threshold * 20,
        )

    def update_noise_floor(self, audio_rms, is_speech):
        """Update the ambient noise estimate from one non-speech frame."""
        if is_speech:
            return
        alpha = (
            self.noise_floor_rise_alpha if audio_rms > self.noise_floor
            else self.noise_floor_fall_alpha
        )
        self.noise_floor = (1 - alpha) * self.noise_floor + alpha * audio_rms
        self.noise_floor = min(self.noise_floor_max, max(self.noise_floor_min, self.noise_floor))

    def adaptive_speech_min_rms(self, tts_is_active):
        """Minimum RMS required to accept a WebRTC-VAD speech frame.

        Applies during ordinary listening as well as barge-in, since ambient
        noise (a TV switching to a noisy restaurant, etc.) can jump at any
        time, not only while TTS is playing. The barge-in case uses a higher
        floor/margin because residual echo adds energy on top of ambient noise.
        """
        min_rms = self.barge_in_min_rms if tts_is_active else self.speech_min_rms
        margin = self.barge_in_margin_rms if tts_is_active else self.speech_margin_rms
        return max(min_rms, self.noise_floor + margin)
    ...
    def is_speech_vad(self, audio_data):
        ...
        try:
            ...
            # Amplitude fallback — gate against the tracked noise floor so a
            # sudden rise in ambient noise doesn't get read as speech.
            rms = np.sqrt(np.mean(audio_data.astype(np.float32) ** 2))
            threshold = max(self.silence_threshold, self.noise_floor * 1.5)
            result = rms >= threshold
            logging.warning(f"VAD exception ({e}), amplitude fallback: RMS={rms:.0f} threshold={threshold:.0f} speech={result}")
            return result
    ...
    def listen_full_duplex(self, stream):
        ...
        while True:
            ...
            if audio_rms < self.adaptive_speech_min_rms(tts_is_active):
                # WebRTC VAD can mistake a burst of ambient noise (or, during
                # TTS, faint residual far-end speech) for voice. Require
                # near-end energy above the current adaptive noise floor.
                is_speech = False
            self.update_noise_floor(audio_rms, is_speech)
...
def restart_service():
    command = "sudo systemctl restart robot.service"
    shell_api.execute_command(command)
...
def main():
    ...
    try:
        ...
        restart_service()
        logging.info("robot.service restart command finished; exiting")
        os._exit(0)
